...
* **Não é possível alterar do modo de importação para o DirectQuery:**  observe que, embora seja possível mudar um modelo do modo DirectQuery para o modo de importação, isso significa que todos os dados necessários devem ser importados. Também não é possível mudar de volta (principalmente devido ao conjunto de recursos sem suporte no modo DirectQuery). Os modelos do DirectQuery em fontes multidimensionais, como SAP BW, também não podem ser alternados de DirectQuery para importação, devido ao tratamento completamente diferente das medidas externas.
